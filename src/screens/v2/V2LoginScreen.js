...
export default function V2LoginScreen({ navigation }) {
  ...
  const join = () => {
    ...
    const onInit = () => { setStatus('connected'); s.off('init', onInit); s.off('error', onErr); };
    const onErr = (e) => { setStatus('error'); setMsg((e && e.message) || 'فشل الاتصال'); s.off('init', onInit); s.off('error', onErr); };
    s.off('init'); s.off('error');
    s.on('init', onInit); s.on('error', onErr);
